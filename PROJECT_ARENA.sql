...
;WITH Q1 AS 
(
	SELECT PM.player_id,email_address,credit_card_type,credit_card_number,
			ROW_NUMBER() OVER(PARTITION BY P.PLAYER_ID ORDER BY CASE 
				WHEN PM.credit_card_type LIKE 'americanexpress' THEN 1
				WHEN PM.credit_card_type LIKE 'mastercard' THEN 2
				WHEN PM.credit_card_type LIKE 'visa' THEN 3
			END) AS RANK_CREDIT
	FROM paying_method AS PM JOIN players AS P
		ON P.player_id=PM.player_id
)
SELECT Q1.player_id,Q1.email_address,Q1.credit_card_type,Q1.credit_card_number
FROM Q1
WHERE Q1.RANK_CREDIT = 1

---)2 דוח המציג את כמות השחקנים אשר נושאים כל סוג כרטיס, לפי כל מגדר וקבוצת גיל 
